#!/usr/bin/env python3
"""
🔴 Hard Refiner 4th - The Foundation Layer (Hard)

Logic:
1. Downbeats: ALWAYS Note.
2. Other Beats: Note if Onset Strength > HARD Threshold (0.7 * local_avg).
   (Lower threshold than Medium to include more onsets).
...
class HardRefiner4th:
    ...
    def run(self):
        logger.info(f"🔴 Starting Hard 4th Refiner...")
        ...
        logger.info(f"✅ Hard 4th Layer Generated: {self.sm_output}")
    ...
    def _generate_4th_layer(self):
        ...
        for i, beat in enumerate(beat_stats):
            ...
            if beat['is_downbeat']:
                ...
                threshold = local_avg * 0.7  # Lower threshold = More notes
    ...
    def _inject_chart(self, measures):
        ...
        # Filter out existing Hard chart
        final_charts = []
        for chart in existing_charts:
            if not re.search(r'^\s*Hard:\s*$', chart, re.MULTILINE | re.IGNORECASE):
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("Usage: python hard_4th.py <input_sm> <output_sm> <analysis_json>")
    else:
        refiner = HardRefiner4th(sys.argv[1], sys.argv[2], sys.argv[3])
        refiner.run()
